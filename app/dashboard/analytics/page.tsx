...
// LUCIDE ICONS (Emojiler yerine kurumsal ikonlar eklendi)
import { 
  Rocket, Briefcase, Zap, BatteryCharging, 
  ChevronDown, ChevronUp, Download, Sparkles, BarChart3
} from 'lucide-react'

// RECHARTS (Modern Veri Görselleştirme Kütüphanesi)
import { 
...
export default function AnalyticsPage() {
  ...
  // --- MODÜL DAĞILIMI (En çok kullanılanı bulma) ---
  ...
  // --- AKILLI KOÇ TAVSİYESİ (Dinamik) ---
  const getSmartAdvice = () => {
    if (totalItems === 0) return "Henüz analiz yapmaya başlamadınız. 'Requirement Generator' ile ilk gereksiniminizi üreterek başlayabilirsiniz."
    
    if (mostUsedModule.includes('Requirement')) return "Analizlerime göre 'Requirement Generator' modülünü yoğun kullanıyorsunuz. Yazılım ekibindeki hata oranını düşürmek için, ürettiğiniz bu gereksinimleri 'Test Case Generator' ile otomatik test senaryolarına dönüştürmenizi öneririm."
    if (mostUsedModule.includes('SQL')) return "Veritabanı işlemleri için 'SQL Generator' modülünü sıkça kullanıyorsunuz. Ürettiğiniz karmaşık sorguların veri güvenliğini teyit etmek için arada bir 'Risk Analyzer' modülüne danışabilirsiniz."
    if (mostUsedModule.includes('Meeting')) return "Toplantı notlarını sürekli 'Meeting Analyzer' ile çözümlüyorsunuz. Çıkan aksiyon maddelerini (Action Items) önceliklendirmek için 'Prioritization Coach' modülünü kullanmak iş akışınızı hızlandıracaktır."
    if (mostUsedModule.includes('Wireframe')) return "Görsel üretimlere (Wireframe) odaklanmış durumdasınız. Bu tasarımların arkasındaki kullanıcı motivasyonunu sağlamlaştırmak için öncesinde 'Persona Generator'ı kullanmak UX kalitenizi %30 artırabilir."
    
    return `Son zamanlarda en çok '${mostUsedModule}' üzerine çalıştınız. Bu verimliliği diğer analiz süreçlerine de yaymak için farklı modülleri keşfedebilirsiniz.`
  }
        ...
        {isExpanded && (
          <div style={{ marginTop: '24px', paddingTop: '20px', borderTop: `1px dashed ${colors.border}`, animation: 'fadeIn 0.3s' }}>
            <div style={{ fontSize: '13px', color: colors.textMuted, lineHeight: '1.6', marginBottom: '16px' }}>
              <strong style={{ color: colors.text, display: 'block', marginBottom: '6px' }}>Stratejik Etki</strong>
              {detailText}
            </div>
            <div style={{ background: isDark ? 'rgba(255,255,255,0.03)' : '#f8fafc', padding: '14px', borderRadius: '8px', fontSize: '11px', fontFamily: 'monospace', color: colors.textMuted, border: `1px solid ${colors.border}` }}>
              <strong style={{ color: colors.text, marginBottom: '6px', display: 'block' }}>Matematiksel Model:</strong>
              {formula}
            </div>
            <button 
  ...
  if (loading) {
     return (
       <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '400px', color: colors.textMuted, flexDirection: 'column', gap: '16px' }}>
         ...
         <div style={{ fontWeight: '600', fontSize: '14px' }}>Metrikler Hesaplanıyor...</div>
       </div>
     )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: colors.textMuted, margin: '6px 0 0' }}>
            Yapay zekanın iş süreçlerinize kattığı operasyonel ve finansal değeri ölçümleyin.
          </p>
        </div>
        <div style={{ display: 'flex', gap: '12px' }}>
          <select 
            ...
            style={{ background: colors.card, color: colors.text, border: `1px solid ${colors.border}`, padding: '10px 16px', borderRadius: '10px', fontSize: '13px', cursor: 'pointer', outline: 'none', fontWeight: '700', boxShadow: isDark ? 'none' : '0 2px 8px rgba(0,0,0,0.03)' }}
          >
          ...
          >
            <Download size={16} /> Yönetici Raporunu İndir
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '20px', alignItems: 'flex-start' }}>
        <KPICard 
          id="time"
          title="Kazanılan Net Zaman"
          value={`${savedHours} Saat`}
          subValue={`~ ${(savedHours / 8).toFixed(1)} İş Günü Kazancı`}
          icon={<Rocket size={24} strokeWidth={1.5} />}
          detailText={
            <>Geleneksel yöntemlerle <strong>{manualHoursTotal.toFixed(1)} saat</strong> sürecek olan <strong>{totalItems} adet</strong> işi, BAI Platform ile sadece <strong>{aiHoursTotal.toFixed(1)} saatte</strong> tamamladınız. Kazanılan <strong>{savedHours} net saat</strong> ile inovasyona odaklanabilirsiniz.</>
          }
        ...
        <KPICard 
          id="money"
          title="Tahmini Finansal Tasarruf"
          value={`${totalSavingsTL.toLocaleString('tr-TR')} TL`}
          subValue="Maliyet Kaçınması (Cost Avoidance)"
          icon={<Briefcase size={24} strokeWidth={1.5} />}
          detailText={
            <>Kazanılan <strong>{savedMinutes} dakikalık</strong> eforun uzman maliyeti üzerinden karşılığı <strong>{totalSavingsTL.toLocaleString('tr-TR')} TL</strong>'dir. Kurumunuz manuel iş gücü maliyetinden büyük oranda tasarruf etmiştir.</>
          }
        ...
        <KPICard 
          id="production"
          title="Toplam İş Çıktısı"
          value={`${totalItems} Adet`}
          subValue="Uçtan Uca SDLC Kapsamı"
          icon={<Zap size={24} strokeWidth={1.5} />}
          detailText={
            <>Bugüne kadar toplam <strong>{totalItems} adet</strong> teknik ve analitik çıktı ürettiniz. Hızlı ve hatasız üretim gücünüz yazılım geliştirme yaşam döngünüzü hızlandırır.</>
          }
        ...
        <KPICard 
          id="quality"
          title="Kredi Verimliliği"
          value="%88"
          subValue="İlk Seferde Başarı (First-Prompt)"
          icon={<BatteryCharging size={24} strokeWidth={1.5} />}
          detailText={
            <>Sisteme gönderdiğiniz isteklerin büyük bölümü ilk seferde başarıya ulaştı. Prompt mühendisliğindeki bu başarınız 'rework' maliyetlerini sıfırlar.</>
          }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(400px, 2fr) minmax(300px, 1fr)', gap: '20px' }}>
        ...
        {/* Üretkenlik Ritmi (Yenilenmiş, Izgaralı ve Modern Recharts) */}
        <div style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: '16px', padding: '24px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px' }}>
            <div>
              <div style={{ fontWeight: '800', color: colors.text, fontSize: '18px' }}>Üretkenlik Ritmi</div>
              <div style={{ fontSize: '13px', color: colors.textMuted, marginTop: '4px' }}>Son 7 gündeki aktif doküman ve analiz üretimi</div>
            </div>
            <div style={{ fontSize: '11px', color: accent, background: accent + '15', fontWeight: '700', padding: '6px 12px', borderRadius: '8px' }}>📊 Günlük Aktivite</div>
          </div>
...
<div style={{ flex: 1, minHeight: '260px', width: '100%' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                ...
                {/* SVG GRADIENT (RENK GEÇİŞİ) TANIMLAMASI */}
                <defs>
                      ...
                      return (
                        <div style={{ background: isDark ? '#27272a' : '#ffffff', border: `1px solid ${colors.border}`, padding: '12px 16px', borderRadius: '10px', boxShadow: '0 8px 25px rgba(0,0,0,0.12)' }}>
                          ...
                          <div style={{ fontSize: '18px', fontWeight: '800', color: accent, display: 'flex', alignItems: 'baseline', gap: '6px' }}>
                            {payload[0].value} <span style={{fontSize:'11px', color:colors.textMuted, fontWeight: '600'}}>Üretim</span>
                          </div>
                        </div>
                      )
                    }
                ...
                {/* Sütunlar (Fill kısmına url(#barGradient) verildi) */}
        ...
        {/* Efor Dağılımı (Progress Barlar) */}
        <div style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: '16px', padding: '24px' }}>
          <div style={{ fontWeight: '800', color: colors.text, fontSize: '18px', marginBottom: '4px' }}>Efor Dağılımı</div>
          <div style={{ fontSize: '13px', color: colors.textMuted, marginBottom: '32px' }}>Modül bazlı kullanım yoğunluğu</div>
          
          <div style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
            {Object.keys(moduleCounts).length === 0 ? (
              <div style={{ fontSize: '13px', color: colors.textMuted, textAlign: 'center', padding: '30px 0', background: isDark ? '#18181b' : '#f8fafc', borderRadius: '12px' }}>Henüz yeterli veri yok.</div>
            ) : (
              Object.entries(moduleCounts).sort((a:any, b:any) => b[1] - a[1]).slice(0, 5).map(([name, count]: any, i) => (
                <div key={i}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                    ...
                    <span style={{ fontSize: '11px', fontWeight: '800', color: accent, background: accent + '15', padding: '4px 8px', borderRadius: '6px' }}>{count} Çıktı</span>
                  </div>
      ...
      {/* Akıllı Tavsiyeler (Glow Efekti ile Premium Görünüm) */}
      <div style={{ 
